...
import { getContainerById, updateContainerById } from 'apiSdk/containers';
...
function ContainerEditPage() {
  ...
  const handleSubmit = async (values: ContainerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateContainerById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<ContainerInterface>({
    initialValues: data,
